...
namespace PokemonReviewApp.Controllers
{
    ...
    public class CountryController : Controller
    {
        ...
        [ProducesResponseType(400)]
        public IActionResult GetOwnerFromCountryId(int countryId) 
        {
            if(!_countryRepository.CountryExists(countryId))
            {
                return NotFound();  
            }
            var owners = _mapper.Map<List<OwnerDto>>(_countryRepository.GetOwnersFromCountryId(countryId));
            if(!ModelState.IsValid)
                return BadRequest(ModelState);
            return Ok(owners);
        }
        ...
        public IActionResult DeleteCountry(int countryId)
        {
            ...
            if (_countryRepository.GetOwnersFromCountryId(countryId).Any())
            {
                ModelState.AddModelError("", "The country have one or some Owner , Couldn't delete");
                return StatusCode(500, ModelState);
            }
            if (!ModelState.IsValid)
            {
            ...
            {
                ModelState.AddModelError("", "Something went wrong while delete country");
            }
            return NoContent();
        }
    }
